...
- **Триггеры запуска:** Workflow запускается автоматически при каждом **push** и **Pull Request** в ветку `main`.

- **Основная задача:** Пайплайн выполняет полный прогон всех UI-тестов в `headless`-режиме на чистом окружении (`ubuntu-latest`).
